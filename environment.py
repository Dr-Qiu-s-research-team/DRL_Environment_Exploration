...
''' local sensing model parameters
'''
OBSTACLE_REWARD = -2.0
GOAL_REWARD = 10.0
DIST_REWARD = 0.1
...
'''
Sep 21:
1. add gazebo_random obstacle generator
'''
...
class Env(object):
    def __init__(self, args, obs_list, source, goal):
        ...
        #    ['voxel_random', 'voxel_constrain', 'plane_random', 'random', 'gazebo_random', 'demo', 'test']
    ...
    '''
    def reset(self, num_obst, num_objs):
    ...
    def _select_obst_generator(self):
        ...
        gen_map = {
            ...
            'gazebo_random': 'ObstGeneratorGazeboRandom',
            'demo': 'ObstGeneratorDemo'
        }
        if self._obst_generation_mode == "random":
        ...
        if curt_obst_generation_mode == 'voxel_random':
            self._obst_generator = obst_gen_class('voxel_random', self.env_size, self.verbose)
        elif curt_obst_generation_mode == 'voxel_constrain':
            self._obst_generator = obst_gen_class('voxel_constrain', self.env_size, self.verbose)
        elif curt_obst_generation_mode == 'plane_random':
            self._obst_generator = obst_gen_class('plane_random', self.env_size, self.verbose)
        elif curt_obst_generation_mode == 'gazebo_random':
            self._obst_generator = obst_gen_class('gazebo_random', self.env_size, self.verbose)
        elif curt_obst_generation_mode == 'demo':
            self._obst_generator = obst_gen_class('demo', self.env_size)
    ...
    def _encoding_2_state_conv_global(self):
        state_placeholder = np.zeros(
            (self.env_size, self.env_size, self.env_size)).astype(np.float32)
        for temp_obst in self.objs_info['obst_list']:
            state_placeholder[
        ...
        return (state_placeholder, self.objs_info['drone_pos'] / float(self.env_size))
    ...
    def _reset_objs(self):
        objs_info = self._obst_generator(num_obst=self.num_obst, num_objs=self.num_objs)
        return objs_info
    '''
...
class ObstGeneratorPlaneRandom(ObstGenerator):
    ...
    def _generate_wall_obst_list(self, wall_normal_axis):
        ...
        # wall_size = np.random.randint(self._env_size - 1) + 1     # range: [1, env_size-1]
...
class ObstGeneratorGazeboRandom(ObstGenerator):
    def __init__(self, mode, env_size, verbose):
        super(ObstGeneratorGazeboRandom, self).__init__(mode, env_size)
        self._verbose = verbose

    def __call__(self, **kwargs):
        obst_list=[]
        assert kwargs['num_objs'] <= 20
        temp = create_objs(kwargs['num_objs'])
        for item in temp:
            temp_obs = np.random.randint(self._env_size, size=3)
            temp_obs[0]=item[0]
            temp_obs[1]=item[1]
            ...
            obst_list.append(temp_obs.astype(np.float32))
        if int(self._verbose) > 2:
            print('The number of obstacles: ' + str(kwargs['num_objs']) + ' including grids ' + str(len(obst_list)))
        goal = None
        # goal = np.asarray((9,9,9))
        while goal is None:
            temp_goal = np.random.randint(self._env_size, size=3)
            # temp_goal = np.asarray((np.random.randint(self._env_size), np.random.randint(self._env_size), np.random.randint(self._env_size // 2))) # lower half of env
            if not self._array_in_list(obst_list, temp_goal):
                goal = temp_goal
        drone_pos = None
        # drone_pos = np.asarray((0,0,0))
        while drone_pos is None:
            # temp_drone_pos = np.random.randint(self._env_size, size=3)
            # temp_drone_pos = np.asarray((np.random.randint(self._env_size), np.random.randint(self._env_size), np.random.randint(self._env_size // 2))) # lower half of env
            temp_drone_pos = np.asarray((np.random.randint(self._env_size), np.random.randint(self._env_size), 0)) # groud of env
            if (not self._array_in_list(obst_list, temp_drone_pos)) \
                and (not np.array_equal(temp_drone_pos, goal)):
                drone_pos = temp_drone_pos
        objs_info = {
...
class ObstGeneratorDemo(ObstGenerator):
    def __init__(self, mode, env_size):
        super(ObstGeneratorDemo, self).__init__(mode, env_size)
    ...
    def __call__(self, **kwargs):
        ...
        # # Environment 1
        if index == 1:
            obst_list.append(np.array([1, 1, 0]).astype(np.float32))
            obst_list.append(np.array([1, 1, 1]).astype(np.float32))

        # # Environment 2
        if index == 2:
            obst_list.append(np.array([1, 1, 0]).astype(np.float32))
            obst_list.append(np.array([1, 1, 1]).astype(np.float32))
            obst_list.append(np.array([2, 1, 0]).astype(np.float32))
            obst_list.append(np.array([2, 1, 1]).astype(np.float32))
        ...
        # # Environment 4
        if index == 4:
            obst_list.append(np.array([0, 1, 0]).astype(np.float32))
            obst_list.append(np.array([0, 1, 1]).astype(np.float32))
            obst_list.append(np.array([1, 2, 0]).astype(np.float32))
            obst_list.append(np.array([1, 2, 1]).astype(np.float32))
        ...
        if index == 5:
            ...
            obst_list.append(np.array([0, 1, 1]).astype(np.float32))
            obst_list.append(np.array([1, 2, 0]).astype(np.float32))
            obst_list.append(np.array([1, 3, 0]).astype(np.float32))

        # Environment 6
        if index == 6:
            obst_list.append(np.array([0, 1, 0]).astype(np.float32))
            obst_list.append(np.array([0, 1, 1]).astype(np.float32))
            obst_list.append(np.array([2, 2, 0]).astype(np.float32))
            obst_list.append(np.array([2, 2, 1]).astype(np.float32))
            obst_list.append(np.array([3, 2, 0]).astype(np.float32))
            obst_list.append(np.array([3, 2, 1]).astype(np.float32))
        drone_pos = None
        while drone_pos is None:
            temp_drone_pos = np.asarray((np.random.randint(4), np.random.randint(4), 0)) # groud of env
            # temp_drone_pos = np.asarray((0, 0, 0))
            if (not self._array_in_list(obst_list, temp_drone_pos)):
                drone_pos = temp_drone_pos
        # drone_pos = np.asarray((0,0,0))
        goal = None
        while goal is None:
            temp_goal = np.asarray((np.random.randint(4), np.random.randint(4), np.random.randint(2)))
            if not self._array_in_list(obst_list, temp_goal) \
                    and (not np.array_equal(drone_pos, temp_goal)):
                goal = temp_goal
        # goal = np.asarray((3,3,1))
        objs_info = {
            "obst_list": tuple(obst_list),
            "goal": goal,
            "drone_pos": drone_pos,
            "drone_pos_start": drone_pos
        }
        return objs_info
